...
public class ArrayExample2 {
	...
	public void shallowCopy() {
		...
		copyArr[2] = 999; // 복사본의 배열 index 2번 변경
	...
	public void deepCopy() {
		...
		/* 2. System.arraycopy(원본배열명, 원본복사 시작 index, 
	 					  복사배열명, 복사배열의 삽입 시작 index, 복사길이); */
	...
	public void createLottoNumber() {
		...
		for(int i = 0; i < lotto.length; i++) {
			...
			for (int x = 0; x < i; x++) {
				...
				if (random == lotto[x]) {
					i--;
					/* i가 1씩 증가할 때마다 난수 한 개 생성
					  btw, 중복 값이 나오면 난수를 다시 생성해야.
					  thus, i값을 인위적으로 감소시켜 난수 발생의 기회를
					  다시 한 번 더 주는 것! */
					break;
					/* 앞쪽에서 중복데이터를 발견하면
					  남은 값은 비교할 필요 없다!
		...
		/* 7) 정렬
		   ...
		   Arrays.sort(배열명) : 배열 내 요소값들 오름차순 정렬 */
		Arrays.sort(lotto);
